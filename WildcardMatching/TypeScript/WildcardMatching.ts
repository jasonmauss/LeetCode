...
const isMatch = (s: string, p: string): boolean => {
    ...
    let inputStringIndex:number = 0;
    let patternStringIndex:number = 0;
    let inputPlaceholder:number = null;
    let patternPlaceholder:number = null;
    ...
    while(inputStringIndex < s.length) {
        ...
        if(p[patternStringIndex] === '*') {
            ...
            if(patternStringIndex === p.length - 1) return true;
            // as long as there are asterisks, keep incrementing the pattern position index
            while(p[patternStringIndex + 1] === '*') patternStringIndex++;
            ...
            let nextPatternChar:string = p[patternStringIndex + 1];
            while(inputStringIndex < s.length && s[inputStringIndex] !== nextPatternChar && nextPatternChar !== '?') inputStringIndex++;
