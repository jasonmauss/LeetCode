...
var tictactoe = function (moves) {
    ...
    var rows = new Array(boardRowsAndCols).fill(0), cols = new Array(boardRowsAndCols).fill(0);
    var isPlayerATurn = true;
    var diag1 = 0, // the diagonal from top left to bottom right
    diag2 = 0; // the diagonal from bottom left to top right
    for (var i = 0; i < moves.length; i++) {
        var increment = isPlayerATurn ? 1 : -1;
        isPlayerATurn = !isPlayerATurn;
        var row = moves[i][0], col = moves[i][1];
        rows[row] += increment;
        cols[col] += increment;
        if (row === col)
            diag1 += increment;
        if (row === (boardRowsAndCols - 1) - col)
            diag2 += increment;
