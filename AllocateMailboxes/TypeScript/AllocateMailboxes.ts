...
const minDistance = (houses: number[], k: number): number => {
    ...
    const minDistanceForHouses = (start:number, end:number) => {
    ...
    const partition = (index:number, mailboxes:number) => {
        if (dp[index][mailboxes] === undefined) {
            if (numHouses - index === mailboxes) {
                ...
                for (let i = index; i <= numHouses - mailboxes; i++) {
                    dp[index][mailboxes] = Math.min(
                        dp[index][mailboxes], 
                        minDistanceForHouses(index + 1, i + 1) +
