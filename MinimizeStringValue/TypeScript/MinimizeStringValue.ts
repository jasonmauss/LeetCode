...
const minimizeStringValue = (s: string): string => {
    ...
    for(let i = 0; i < charCounts['?']; ++i) {
        ...
        for(let j = 97; j <= 122; ++j) {
            ...
            if(!charCounts[tempCharProperty]) charCounts[tempCharProperty] = 0;
    ...
    for(let i = 0; i < inputStringAsArray.length; ++i) {
        if(inputStringAsArray[i] === '?') inputStringAsArray[i] = replacementChars.pop();
    }
