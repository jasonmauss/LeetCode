...
const reformat = (s: string): string => {
    ...
    let numsOnly = s.replace(/[^\d]/g, '').split('');
    let charsOnly = s.replace(/[0-9]/g, '').split('');

    if((numsOnly.length === 0 && charsOnly.length > 1) || (charsOnly.length === 0 && numsOnly.length > 1)) return '';

    if(Math.max(numsOnly.length, charsOnly.length) - Math.min(numsOnly.length, charsOnly.length) > 1) return '';
