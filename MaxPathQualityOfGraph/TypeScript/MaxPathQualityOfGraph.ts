...
const dfs = (cur, time, sum) => {
    ...
    if (time < 0) return;
    depth[cur]++;
    if (depth[cur] == 1) sum += value[cur];
    if (cur == 0) res = Math.max(sum, res);
    for (const [child, cost] of graph[cur]) dfs(child, time - cost, sum);
    depth[cur]--;

};

const maximalPathQuality = (values: number[], edges: number[][], maxTime: number): number =>{
