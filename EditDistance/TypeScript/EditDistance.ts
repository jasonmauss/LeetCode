...
const char = (s: string, inx: number) => (inx < 0 || inx + 1 > s.length ? undefined : s[inx]);

// Replace a char at a specific index.
const replaceChar = (i: { s: string; inx: number; char: string }) =>
  i.s.slice(0, i.inx) + i.char + i.s.slice(i.inx + 1);

// Insert a char at a specific index; can append a char too.
const insertChar = (i: { s: string; inx: number; char: string }) =>
  i.s.slice(0, i.inx) + i.char + i.s.slice(i.inx);

// Deletes the char at a specific index.
const deleteChar = (i: { s: string; inx: number }) => i.s.slice(0, i.inx) + i.s.slice(i.inx + 1);

// Calculates the first mismatch between two strings. Returns the index and the
// actual and required characters.
const mismatch = (i: { start: string; goal: string }) => {
  ...
  for (let j = 0; j <= maxInx; j++) {
    ...
    if (required !== actual) return { inx: j, required: required, actual: actual };
  }
  return undefined;
};
...
function* nextEdit(i: { start: string; goal: string }) {
  const m = mismatch(i);
  if (m !== undefined) {
    // replace
    ...
    // insert
    if (m.required !== undefined) yield insertChar({ s: i.start, inx: m.inx, char: m.required });
  }
}
...
const solve = (i: {
  ...
  let afterEdit: Set<string> = new Set();
...
const minDistance = (start: string, goal: string): number => {
