...
const buddyStrings = (s, goal) => {
    ...
    if (s === goal) {
        ...
        for (let i = 0; i < s.length; i++) {
            if (s[i] !== goal[i]) {
                if (charOnePos === Number.MIN_SAFE_INTEGER)
                    charOnePos = i;
                else if (charTwoPos === Number.MIN_SAFE_INTEGER)
                    charTwoPos = i;
                else
                    return false;
            }
        }
        if (charOnePos === Number.MIN_SAFE_INTEGER || charTwoPos === Number.MIN_SAFE_INTEGER)
            return false;
