...
const swimInWater = (grid: number[][]): number => {
    ...
    const dfs = (root:number, child:number) => {
        if(root < 0 || root > gridLength -1 || child < 0 || child > gridLength -1 || time < grid[root][child] || visited.has(root * gridLength + child)) return;
