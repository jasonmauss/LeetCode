...
const canSit = (row, arrangement, newStudent) => {
...
const maxStudents = (seats) => {
    ...
    const classroom = seats.map((row) => (row.reduce((a, c, i) => c === '#' ? a : (a | (1 << i)), 0)));
        ...
        while (queue.length > 0) {
            ...
            for (let arrangement of queue) {
                ...
                for (let [prevArrang, count] of dp[row]) {
                    if (conflicts(prevArrang, arrangement))
                        continue;
                ...
                for (let i = 1; i < lastPos; i <<= 1) {
                    if (canSit(classroom[row], arrangement, i))
                        next.push(arrangement | i);
                }
            }
            queue = next;
            numStudents++;
