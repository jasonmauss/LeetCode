...
const conflicts = (prev:number, curr:number): number => {
...
const canSit = (row:number, arrangement:number, newStudent:number): boolean => {
    return row & newStudent && !(arrangement & newStudent) && !(arrangement & (newStudent << 1)) && !(arrangement & (newStudent >> 1));
}

const maxStudents = (seats: string[][]): number => {
