...
const minimumDistance = (word: string): number => {
    ...
    const distanceCalc = (firstCharPos:number, secondCharPos:number):number => {
    ...
    const distances:Uint16Array = new Uint16Array(word.length - 1);
    ...
    for(let i:number = 2; i < word.length; ++i) {
        sum += distance;
        ...
        for(let j = 0; j < i - 1; ++j) {
            ...
            if(otherMin < minDistance) minDistance = otherMin;
        }
