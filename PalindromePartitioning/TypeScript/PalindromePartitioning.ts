...
const partition = (s: string): string[][] => {
	...
	const isPanlindrome = (stringValue:string): boolean => {
	...
	const palindromeHelper = (stringVal, stringArray) => {
		...
		if(stringVal.length === 0 && stringArray.length !== 0) { P.push(stringArray); return; }

		for(let i = 0; i < stringVal.length; i++) {
            
            const stringPart = stringVal.substring(0, i + 1);

			if(isPanlindrome(stringPart)) palindromeHelper(stringVal.substring(i+1), [...stringArray, stringPart]);
